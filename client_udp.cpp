...
int main(int argc, char *argv[])
{
    ...
    asyncSocket.Init("127.0.0.1", 4001);

    //LOG_INFO("Connection Success !!");
    char msg0[] = "Bonjour Nathan!";
    std::cout <<">>" << msg0  << std::endl;
    asyncSocket.SendTo(msg0, sizeof(msg0));
    
    char buffer[255];
    asyncSocket.ReceiveFrom(buffer, sizeof(buffer));
    std::cout << "\t\t\t\tServer: " << buffer << std::endl;

    char msg[] = "Mieux en UDP!";
    std::cout <<">>" << msg  << std::endl;
    asyncSocket.SendTo(msg, sizeof(msg));
    sleep(2);
    asyncSocket.ReceiveFrom(buffer, sizeof(buffer));
    std::cout << "\t\t\t\tServer: " << buffer << std::endl;
    sleep(2);
    ...
    asyncSocket.SendTo(msg2, sizeof(msg2));
    asyncSocket.ReceiveFrom(buffer, sizeof(buffer));
    std::cout << "\t\t\t\tServer: " << buffer << std::endl;
    sleep(2);
